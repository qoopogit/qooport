 ...
 * specified in the JNA interface * User32.java * * @author Pete S *
 *
 */
public class WInJnaUtil {
    ...
    public static Rectangle getWindowRect(Pointer hWnd) throws Exception {
        if (hWnd == null) {
            throw new Exception("Failed to getWindowRect since Pointer hWnd is null");
        }
    ...
    public static Rectangle getWindowRect(String startOfWindowName) throws Exception {
        Pointer hWnd = getWinHwnd(startOfWindowName);
        if (hWnd != null) {
            return getWindowRect(hWnd);
        } else {
            throw new Exception("Failed to getWindowRect for \"" + startOfWindowName + "\"");
        }
    }
...
//    }
    ...
    public static Punto getCursorPosicion() {
        WinDef.POINT punto = new WinDef.POINT();
        user32.GetCursorPos(punto);
        return new Punto(punto.x, punto.y);
    }

    public static BufferedImage getScreenShot(Rectangle cuadro) {
        try {
            return plugin.jna.windows.screen.WinScreenShot.getScreenshot(cuadro);
        } catch (Exception e) {
            return null;
        }
    ...
    public static void lanzarAppEscaparIDSesion0(String cmd) {
        LoaderService load = new LoaderService();
        load.lanzarAppEscaparIDSesion0(cmd);
    }

    public static void instalarServicio(String cmd, String parametros, String nombre, boolean ar64) {
        InstalarServicio serv = new InstalarServicio();
        serv.extraer(ar64);
        serv.detenerServicio(nombre);
        serv.desinstalarServicio(nombre);
        serv.instalar(cmd, parametros, nombre);
        serv.iniciar(nombre);
    }
}
