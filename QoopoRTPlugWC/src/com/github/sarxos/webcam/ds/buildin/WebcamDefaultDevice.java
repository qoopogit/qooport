...
//    };
    ...
    static {
        WebcamResolution[] list = WebcamResolution.values();
        DIMENSIONS = new Dimension[list.length];
        int i = 0;
        for (WebcamResolution item : list) {
            DIMENSIONS[i] = item.getSize();
            i++;
        }
    }
    // @formatter:on
...
//        int length = size.width * size.height * 10;
